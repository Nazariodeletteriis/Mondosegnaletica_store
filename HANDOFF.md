...
- ✅ **GIT: 2 commit pushati su `main`** — `8668a58` (coming-soon) e `397893a` (output epanza sessione 12). **Working tree pulito, allineato a `origin/main`. `deploy/` non è più untracked.**
  - ✅ **TRAPPOLA GIT — RISOLTA IN MODO PERMANENTE (28.07, checkpoint).** Il push falliva con **403** perché in `gh` sono loggati **DUE account**: **`Nazariodeletteriis` = PERSONALE** (proprietario di questo repo) e **`nazariodeletteriis-it` = AZIENDALE** (attivo di default, **senza accesso** a Mondosegnaletica). **Mondosegnaletica sta sull'account PERSONALE.**
    - **SOLUZIONE ADOTTATA: remote passato da HTTPS a SSH** → `origin = git@github.com:Nazariodeletteriis/Mondosegnaletica_store.git`. Funziona perché `ssh -T git@github.com` autentica **già** come `Nazariodeletteriis` con la chiave **`~/.ssh/id_ed25519_github`**. Il push usa quindi sempre l'account personale, **solo per questo repo**, mentre la config `gh` globale resta sull'account aziendale per tutti gli altri progetti. **NON serve più `gh auth switch` a ogni push.**
    - ✔️ **Verificato dopo il cambio**: `git ls-remote` OK, `git push --dry-run` OK, account `gh` attivo ancora `nazariodeletteriis-it` (non alterato).
    - 🚫 **MISURATO, DA NON RITENTARE**: `gh` **2.45.0 non fa lookup del token per username**. Passando `username=Nazariodeletteriis` al credential helper restituisce **vuoto** — è questo il motivo per cui mettere l'utente nell'URL HTTPS del remote falliva con **"could not read Password"**. L'unica via HTTPS sarebbe stata `gh auth switch` a ogni push, ora **superata da SSH**.
